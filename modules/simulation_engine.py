...
# OpenMM imports - explicit for clarity and IDE support
from openmm import (
    Simulation, Context, Platform, Vec3, CustomExternalForce,
    LangevinIntegrator, VerletIntegrator, VariableLangevinIntegrator,
    MonteCarloBarostat, MonteCarloAnisotropicBarostat
)
from openmm.app import (
    PDBFile, Topology, DCDReporter, StateDataReporter, PDBReporter,
    ForceField, PME, NoCutoff, CutoffNonPeriodic, CutoffPeriodic,
    Simulation as AppSimulation, GromacsTopFile, GromacsGroFile,
    AmberPrmtopFile, AmberInpcrdFile
)
from openmm.unit import (
    nanometers, picoseconds, angstroms, kelvin, bar, kilojoule_per_mole,
    kilojoule, atom, Quantity
)
...
DEFAULT_TIMESTEP: float = 0.002  # picoseconds
DEFAULT_FRICTION: float = 1.0  # 1/picoseconds
DEFAULT_NONBONDED_CUTOFF: float = 1.0  # nanometers
DEFAULT_REPORTER_FREQ: int = 1000  # steps

# Standard force field files
LEAPRC_PROTEIN: str = "leaprc.protein.ff14SB"
LEAPRC_WATER: str = "leaprc.water.fb3"


def get_rmsd(pdb_ref: str, pdb_prb: str) -> float:
    """
    Calculate the Root Mean Square Deviation (RMSD) between two molecular structures.

    Computes RMSD after optimal alignment of the probe molecule to the reference molecule
    using the RDKit library.

    Args:
        pdb_ref (str): Path to the PDB file of the reference molecule.
        pdb_prb (str): Path to the PDB file of the probe molecule.

    Returns:
        float: The RMSD between the reference and probe molecules after alignment (Angstroms).

    Raises:
        FileNotFoundError: If either PDB file cannot be loaded.
        ValueError: If molecules cannot be properly aligned.

    Example:
        >>> rmsd = get_rmsd("ref_structure.pdb", "probe_structure.pdb")
    ...
    if prbMol is None or refMol is None:
