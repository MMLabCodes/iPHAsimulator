...
A python class works by initialising an instance of itself. The class in **filepath_manager** is **PolySimManage**.

Before initialising the instance of filepath manager (**PolySimManage**) it is important to be in the main directory of the project after cloning the project. This main directory will have a subdirectory called **modules**.
This is **extremely important** as this class will initialise a series of subdirectories to store files generated with this code and running it another directory will create directories in the wrong place.

To initialise an instance of **PolySimManage** in jupyter notebook or the python interpreter you must pass the base directory path to **PolySimManage** which is generated with **os.getcwd**.
...
.. code-block:: python
   ...
   import os
   main_dir = os.getcwd()
   manager = PolySimManage(main_dir)

If you see an import error, this means that you are launching your jupyter notebook or python interpreter from the main directory and you **must** navigate to that directory before continuing.
...
If you executed the code above, you will have an instance of the **PolySimManage** class called **manager**. To use this instance you will want to call **manager.some_method** or **manager.some_attribute**.
